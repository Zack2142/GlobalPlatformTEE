    ...
    TEEC_Context* context)
{
    ...
    if( ret == -1){
        ...
        context->imp.s_errno = errno;
        return TEEC_ERROR_GENERIC;
    } else {
        context->imp.fd = ret ;
        context->imp.session_count = 0;
        context->imp.shared_mem_cnt = 0;
...
void TEEC_FinalizeContext(TEEC_Context* context)
{
    ...
    if(context->imp.session_count != 0) {
        printf("warning: pending open sessions %d\n",
                    context->imp.session_count);
    }

    if(context->imp.shared_mem_cnt != 0) {
        printf("warning: unreleased shared memory blocks %d\n",
                    context->imp.shared_mem_cnt);
    }

    if( (context->imp.session_count == 0) && (context->imp.shared_mem_cnt == 0)) {
...
{ // TODO OPEN SESSION
    ...
    switch(connectionMethod) {
        case TEEC_LOGIN_PUBLIC: {
            if(connectionData != NULL) {
                printf("TEEC_OpenSession : connection method requires empty connection data\n");
                return TEEC_ERROR_BAD_PARAMETERS;
            }
            break;
        }
        ...
        case TEEC_LOGIN_USER_APPLICATION: {
            ...
            printf("TEEC_OpenSession : connection method is not implemented \n");
        ...
        case TEEC_LOGIN_GROUP_APPLICATION: {
            if(connectionData == NULL) {
                printf("TEEC_OpenSession : connection method requires valid connection data\n");
                return TEEC_ERROR_BAD_PARAMETERS;
            }
            return TEEC_ERROR_NOT_IMPLEMENTED;
            break;
    ...
    ret = ioctl(context->imp.fd,
                TEE_CLIENT_IOCTL_SES_OPEN_REQ, &ses_open);
    ...
    context->imp.session_count++;
    session->imp.operation_cnt = 0;
    session->imp.session_id = ses_open.session_id;
    session->imp.service_id = *destination;
...
{// TODO INVOKE COMMAND
    ...
    enc.service_id = session->imp.service_id ;
        ...
        for(param_count = 0; param_count < 4; param_count++) {
            if( (param_types[param_count] == TEEC_VALUE_INPUT) ||
                ...
                ret = ioctl(session->imp.device->fd,
                            TEE_CLIENT_IOCTL_ENC_UINT32, &enc);
                if (ret) {
                    if(returnOrigin)
                        *returnOrigin = TEEC_ORIGIN_API;
                ...
                if(!operation->params[param_count].memref.parent) {
                    ...
                    printf("TEEC_InvokeCommand: memory reference parent is NULL\n");
    ...
    if(operation->paramTypes != 0) {
        for(param_count = 0; param_count < 4; param_count++) {
            ...
            if((param_types[param_count] == TEEC_VALUE_INOUT) ||
                ...
                ret = ioctl(session->imp.device->fd,
                            TEE_CLIENT_IOCTL_DEC_UINT32, &enc);
                if (ret) {
                    if(returnOrigin)
                        *returnOrigin = TEEC_ORIGIN_API;
                    session->imp.s_errno = errno;
...
decoding data in client driver failed\n");
                ...
                ret = ioctl(session->imp.device->fd,
                            TEE_CLIENT_IOCTL_DEC_ARRAY_SPACE, &enc);
                if (ret){
                    if(returnOrigin)
                        *returnOrigin = TEEC_ORIGIN_API;
                    session->imp.s_errno = errno;
...
operation_release:
    ...
    rel_ret = ioctl(session->imp.device->fd,
                    TEE_CLIENT_IOCTL_OPERATION_RELEASE, &enc);
    if (rel_ret){
